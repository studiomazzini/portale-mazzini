  ...
  const save=async f=>{
    try{
      if(modal.mode==="add"){
        ...
        // Aggiorna email di login su Supabase se cambiata
        const utentePre=users.find(u=>u.id===f.id);
        if(f.email&&utentePre&&f.email!==utentePre.email&&isRealEmail(f.email)){
          try{
            await sb("/auth/v1/admin/users/"+f.id,{method:"PATCH",svc:true,body:{email:f.email}});
